...
#=============================================================================
#
# Part 2, start from E, find 'a'.  Going backwards changes the definition of
# reachable (down by 1 instead of up).  Search can be pruned once any distance
# is longer than the shortest known 'a'.
...
sub checkForA($mc, $dist, $r, $c)
{
    if ( $mc eq 'a' && $dist < $ClosestA )
    {
        say "Found a at ($r, $c), dist=$dist";
        $ClosestA = $dist;
    }
}
    ...
    while ( ++$VisitCount < $SIZE )
    {
        ...
        if ( $row > 0 )
        {
            ...
            if ( reachable($me, $n, $map ) )
            {
                ...
                checkForA($nc, $dist->[$row-1][$col], $row-1, $col);
            }
        }
        if ( $row < $HEIGHT-1 )
        {
            ...
            if ( reachable($me, $s, $map) )
            {
                ...
                checkForA($sc, $dist->[$row+1][$col], $row+1, $col);
            }
        }
        if ( $col < $WIDTH-1 )
        {
            ...
            if ( reachable($me, $e, $map) )
            {
                ...
                checkForA($ec, $dist->[$row][$col+1], $row, $col+1);
            }
        }
        if ( $col > 0 )
        {
            ...
            if ( reachable($me, $w, $map) )
            {
                ...
                checkForA($wc, $dist->[$row][$col-1], $row, $col-1);
            }
        }
...
_search( $EndPoint[0], $EndPoint[1], \@Map, \@Dist);
say "MINDIST= $MinDist";
say "Endpoint= $Dist[ $EndPoint[0] ][ $EndPoint[1] ]";
# showDist(\@Dist, \@Map);
showMap(\@Map);
say "Closest A is ", $ClosestA;
showDist(\@Dist, \@Map);
